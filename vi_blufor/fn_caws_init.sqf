...
// Actions
_cawsOnAction = _vehicle addAction [localize "STR_BLND_Caws2_ON", "\VI_blufor\noScript.sqf", '(_this select 0) setVariable ["caws", true, false]; playSound "onlin";', 0, false, false, "", 'player in (crew _target) && !(_target getVariable ["caws", false])'];
_cawsOffAction = _vehicle addAction [localize "STR_BLND_Caws2_OFF", "\VI_blufor\noScript.sqf", '(_this select 0) setVariable ["caws", false, false]', 0, false, false, "", 'player in (crew _target) && _target getVariable ["caws", false]'];
...
while { alive _vehicle } do {
	if (_vehicle call _fnc_caws) then {
		...
		if ((_enginedamageleft == 1) && (time - _engineleftWarning >= heightWarningTime)) then {
			...
			sleep 3; playSound ["LEFblue", true];
		};
		};
